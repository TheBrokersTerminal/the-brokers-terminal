...
serve(async (req) => {
  // Allow CORS from the admin panel
  ...
  try {
    ...
    if (test_email) {
      // TEST MODE — send only to the specified email, no DB lookup needed
      recipients = [{ email: test_email, first_name: 'Test' }]
    } else {
      // LIVE MODE — fetch all active subscribers
      const { data: users, error } = await supabase
        .from('users')
        .select('email, first_name, status, firms(status, access)')
        .eq('status', 'active')
        .not('firm_id', 'is', null)

      if (error) throw error

      // Only active firms, with access matching the asset class
      ...
      recipients.map(async (u: any) => {
        ...
        const res = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            to: u.email,
            subject: `▸ New ${assetLabel} ${typeLabel} — ${title}`,
            html: emailHtml(firstName, title, reportType, assetClass, publishedDate),
          }),
        })
        if (!res.ok) {
